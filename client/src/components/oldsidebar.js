...
async onTrigger(event) {
  ...
  // Loop and push the checked CheckBox value in Array.
   for (var i = 0; i < chks.length; i++) {
    ...
    //var pro_name =  window.location.search.split('name=')[1]
    this.state.data=window.location.search.split('name=')[1] 
      
    await axios.get('http://localhost:8000/predictview/',{params:{text:this.state.data,keywords:keywords}}).then((response) => {
...
componentDidMount(){
  ...
  this.setState({loading:true})
  axios.get('http://localhost:8000/predictview/',{params:{text:this.state.data,keywords:"mobile"}}).then((response) => {
  this.setState({counts:response.data})
